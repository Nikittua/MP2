...
//сам двоичный поиск
vector<Flight> binarySearch(vector<Flight>& v, const string& key) {
    int left = 0;
    int right = v.size() - 1;
    ...
    while (left <= right) {
        ...
        if (v[mid].getCompanyName() == key) {
            // Найден элемент с ключом, продолжаем двигаться влево для поиска нижней границы
            while (mid > 0 && v[mid - 1].getCompanyName() == key) {
                mid--;
            }

            // Найдена нижняя граница, собираем элементы вектора до верхней границы
            vector<Flight> result;
            while (mid <= right && v[mid].getCompanyName() == key) {
        ...
        else if (v[mid].getCompanyName() < key) {
    ...
    return vector<Flight>();  // Искомый элемент не найден, возвращаем пустой вектор
}
